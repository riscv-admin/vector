...
```math
\mathbf{C} = \mathbf{A} S^{\oplus} (\mathbf{u} S_{\otimes} \mathbf{v}) = \left\langle D, M, N, C[i,j] = \left\{ \begin{array}{c} A[i,j] \oplus U_i \otimes v_j \end{array} \right. \right\rangle.
```
